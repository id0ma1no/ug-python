...
class Graphviz(object):
    def node(self, obj):
        ...
        if isinstance(obj, (list, tuple)):
            label = "|".join(
                "<f%d> %s" % (i, repr(v) if issimple(v) else "*")
            ...
            label = "|".join(
                "<f%s> %s"
                % (key, trans(key) + ":" + repr(val) if issimple(val) else key)
    ...
    def link(self, obj1, obj2, attr, is_property=False):
        ...
        text = 'obj_%d -> obj_%d[label="%s",style=%s];' % (
            id(obj1),
            id(obj2),
            attr,
            style,
    ...
    def _graphviz(self, obj):
        ...
        if (
            len(class_names & self.expand_classes) > 0
            or klass in self.expand_once_classes
            and klass not in self.expanded
        ):
...
class GraphvizMPLTransform(Graphviz):
    ...
    def format_ndarray(self, obj):
        if obj.ndim == 2:
            html = "".join(
                "<tr>%s</tr>" % "".join("<td>%g</td>" % v for v in row) for row in obj
            )
